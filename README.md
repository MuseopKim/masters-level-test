...
1부터 100까지의 랜덤한 수를 발생시켜 스트라이크, 볼, 안타, 아웃 중 한가지로 결과가 판정 되도록 하여 최종 결과인 안타의 총 횟수를 도출하는 실행 과정을 만들어 보았습니다.
...
3. 구성 된 경기 정보를 바탕으로 경기를 진행하며, 실행 버튼을 클릭할 때마다 타율을 기반으로 하여 판정을(스트라이크, 볼, 아웃, 안타) 도출 합니다.
...
4. 판정이 결정 되면 결정 된 판정을 매개로 스탯을 증가 시키는 메소드를 실행 합니다. 이때 증가 된 스탯으로 인해 3 아웃이 될 경우, 회초/회말 및 공수 전환, 또는 경기를 종료 시키는 함수를 호출 합니다.
...
6. 모든 메뉴의 활성화와 기능의 실행은 버튼 이벤트를 기반으로 진행 되며, 엔터로 게임을 진행하는 기능을 추가 하였습니다.
...
STEP-2에서 구현한 기능들을 바탕으로 라인업과 경기 결과가 실시간으로 화면에 반영 되는 기능을 구현 하였습니다. STEP-2의 동작 방식에 몇 가지 값이 누적 되는 변수들을(삼진, 투구, 안타) 추가 하였고, 각각의 값이 실시간으로 화면에 출력이 되도록 했습니다.
<br/>
...
- 각 라운드의 회초/회말의 기록을 화면에 남기기 위해 해당 라운드에서만 누적 되다가 라운드 종료시 초기화 되는 스코어 변수
- 각 팀의 삼진, 안타, 투구 횟수를 누적하기 위한 변수
